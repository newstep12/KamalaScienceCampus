 ...
 *              administrator prints from Admin → People and on a document they
 *              issue; a student printing their own card still gets the blank
...
function signature_tables_ready(): bool
{
    ...
    if ($ready === null) {
        try {
            ...
            scalar('SELECT 1 FROM documents WHERE 1 = 0');
            $ready = true;
...
const SIGNATURE_USES = ['id_card', 'document'];
...
function delete_signature(int $id, int $actorId): void
{
    ...
    q('UPDATE documents SET signature_id = NULL WHERE signature_id = ?', [$id]);
    q('DELETE FROM signatures WHERE id = ?', [$id]);
    log_activity($actorId, 'signature_delete', $sig['label']);
}

/**
 * Carry the single signature the portal used to hold in settings into the
 * library, so an upgrade does not lose one already in use.
 *
 * It arrives released to everyone, because that is what it was doing before
 * this table existed: a card printed by anybody carried it. Narrowing that
 * silently would change what the campus's cards look like without anyone
 * asking for it — the Signatures page says plainly how to narrow it.
 *
 * Idempotent: the old setting is cleared as it is carried over, so a second
 * run finds nothing to do.
 */
function migrate_legacy_signature(?int $actorId = null): bool
{
    $path = setting('id_card_signature_path');
    if (!$path) {
        return false;
    }
    // Only if the file is really still there — a path left behind by a deleted
    // upload would otherwise become a library entry that shows a broken image.
    if (resolve_upload($path) === null) {
    ...
    q(
        'INSERT INTO signatures (label, owner_name, owner_name_ne, owner_title, file_path,
                                 release_scope, note, uploaded_by, released_at, released_by)
         VALUES (?, ?, ?, ?, ?, \'everyone\', ?, ?, NOW(), ?)',
        [
            t('signature_legacy_label'),
            setting('id_card_chief_name', t('desig_campus_chief')),
            setting('id_card_chief_name_ne'),
            setting('id_card_chief_title', 'campus_chief'),
            $path,
            t('signature_legacy_note'),
            $actorId,
            $actorId,
        ]
    );
    $id = (int) db()->lastInsertId();
    set_setting('signature_id_card', (string) $id);
    set_setting('id_card_signature_path', null);
    log_activity($actorId, 'signature_migrate', (string) $id);
    return true;
}
...
function signature_actions(): array
{
    return [
        ...
        'signature_edit', 'signature_delete', 'signature_migrate',
        'document_issue', 'document_delete',
    ];
}

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload') {
        ...
        $title = in_array($_POST['owner_title'] ?? '', designations(), true)
            ? (string) $_POST['owner_title'] : 'campus_chief';
...
// An upgrade from the version that kept one signature in the settings table
// brings it into the library the first time this page is opened, so nothing
// already printing on cards stops printing.
if (migrate_legacy_signature((int) $admin['id'])) {
    flash('info', t('signature_migrated'));
    header('Location: ' . portal_url('/admin/signatures.php'));
    exit;
}
    ...
    <div class="p-field-row">
      ...
      <div class="p-field">
        ...
        <select id="new-title" name="owner_title">
          <?php foreach (designations() as $d): ?>
            <option value="<?= e($d) ?>" <?= $d === 'campus_chief' ? 'selected' : '' ?>><?= e(designation_label($d)) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    </div>

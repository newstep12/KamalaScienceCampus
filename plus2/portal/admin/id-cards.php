 ...
 *
 * Who is printed is the filters (students or staff, class, group, printed or
 * not) less anyone unticked in the list; how is the layout, the faces, the
...
$class   = (int) $get('class');
$class   = in_array($class, class_levels(), true) ? $class : 0;
$group   = study_group($get('group'));
...
$here = static function (array $changes = []) use ($who, $class, $group, $printed, $from, $layout, $sides, $orient, $guides, $skip): string {
    $q = array_merge([
        'who' => $who, 'class' => $class ?: null, 'group' => $group, 'printed' => $printed,
...
if ($who === 'students') {
    ...
    $sql .= " AND role IN ('teacher', 'admin')";
}
// A class or a group is a set of students: choosing one leaves staff out.
if ($who !== 'staff' && $class) { $sql .= ' AND class_level = ?'; $par[] = $class; }
if ($who !== 'staff' && $group) { $sql .= ' AND study_group = ?'; $par[] = $group; }
// A class register's order: class, section, roll number. Staff after the
// students, by name.
$sql .= " ORDER BY (role = 'student') DESC, class_level, section, CAST(roll_no AS UNSIGNED), roll_no, full_name, id";
$rows = all($sql, $par);
...
// Every student printed carries a number (see id-card.php); one who became a
// student some other way gets theirs now, as their card is drawn — and only
// if it is, not for everyone the list happens to show.
foreach ($batch as $k) {
    if ($rows[$k]['role'] === ROLE_STUDENT && empty($rows[$k]['student_no'])) {
        assign_student_no((int) $rows[$k]['id']);
        $rows[$k]   = one('SELECT * FROM users WHERE id = ? LIMIT 1', [(int) $rows[$k]['id']]) ?? $rows[$k];
        $prints[$k] = card_fingerprint($rows[$k]);
    }
}

// The design, the signers and the school are the same on every card and are
      ...
      <?php if ($who !== 'staff'): ?>
        ...
        <div class="p-field">
          ...
          <select id="group" name="group">
            ...
            <?php foreach (study_groups() as $gr): ?>
              <option value="<?= e($gr) ?>" <?= $group === $gr ? 'selected' : '' ?>><?= e(study_group_label($gr)) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
      <?php endif; ?>
              ...
              <tr class="<?= $in ? '' : 'idcb-off' ?>">
                ...
                <td>
                  <?php if ($u['role'] === ROLE_STUDENT): ?>
                    <?= $u['class_level'] ? e(class_with_section((int) $u['class_level'], $u['section'] ?? null)) : '—' ?>
                    <div class="idcb-sub">
                      <?= e(implode(' · ', array_filter([
                          study_group($u['study_group'] ?? null) ? study_group_label($u['study_group']) : null,
                          !empty($u['roll_no']) ? t('id_card_roll') . ' ' . localize_digits((string) $u['roll_no']) : null,
                      ]))) ?>
                    </div>
                  <?php else: ?>

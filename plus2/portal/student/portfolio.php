...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST['form'] ?? '') === 'profile') {
        ...
        // What a student keeps up to date for their card: section, roll
        // number and the guardian to ring. The class itself is the school
        ...
        q(
            ...
            [
                ...
                // the card.
                // Staff only: a student's form does not carry them, and what
                // is already stored is kept rather than blanked.
      ...
      <?php if ($user['role'] === ROLE_STUDENT): ?>
        <div class="p-field-row">
          <div class="p-field">
            ...
            <input type="text" id="class_level" value="<?= e($user['class_level'] ? class_label((int) $user['class_level']) : '—') ?>" disabled>
            <span class="hint"><?= te('class_set_by_office') ?></span>
          </div>
          <div class="p-field">
            <label for="section"><?= te('section') ?> <span class="hint"><?= te('optional') ?></span></label>
            <input type="text" id="section" name="section" maxlength="10" value="<?= e($user['section'] ?? '') ?>"
        ...
        <div class="p-field-row">
          <div class="p-field">
            ...
            <input type="text" id="guardian_name" name="guardian_name" maxlength="120" value="<?= e($user['guardian_name'] ?? '') ?>">
          </div>
          <div class="p-field">
            <label for="guardian_phone"><?= te('guardian_phone') ?></label>
            <input type="tel" id="guardian_phone" name="guardian_phone" maxlength="30" value="<?= e($user['guardian_phone'] ?? '') ?>">
          </div>
        </div>
      <?php endif; ?>
